import {
  ...
  DentistProcedureCreateResponse,
...
export default class DentistProcedureService {
  ...
  async listProcedureToDentistAndPatient(): Promise<
    DentistProcedureSelectResponse[]
  > {
    try {
  ...
  async findProcedureToDentistAndPatientById(
    ...
    try {
      let valid = dentistId && patientId && procedureId

      if (!valid) {
        throw "Todos os parâmetros devem ser informados"
      }

      return await this.dentistProcedureRepository.findById(
        procedureId,
        dentistId,
        patientId
      )
  ...
  ): Promise<DentistProcedureCreateResponse> {
    try {
      let valid =
        dentistProcedure.dentistId &&
  ...
  async removeProcedureFromDentistAndPatient(
  ...
  async updateProcedureFromDentistAndPatient(
